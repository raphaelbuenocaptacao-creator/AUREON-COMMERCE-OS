...
const CACHE_NAME = `${CACHE_PREFIX}shell-v11-private-vary-range-safe`;
...
function isSafeResponse(response) {
  ...
  if (varyKeys.some((key) => key === '*' || key === 'cookie' || key === 'authorization' || key === 'range' || key === 'if-range')) return false;
  return true;
}
